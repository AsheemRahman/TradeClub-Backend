...
class OrderController implements IOrderController {
    ...
    failedOrder = asyncHandler(async (req: Request, res: Response) => {
        const { sessionId } = req.body;
        if (!sessionId) {
            res.status(STATUS_CODES.BAD_REQUEST).json({ status: false, message: "Session ID is required" });
            return;
    ...
    getPurchasedCourse = asyncHandler(async (req: Request, res: Response) => {
        const userId = req.userId;
        if (!userId) {
            res.status(STATUS_CODES.UNAUTHORIZED).json({ status: false, message: ERROR_MESSAGES.UNAUTHORIZED || 'Unauthorized access', });
            return;
    ...
    subscriptionCheckout = asyncHandler(async (req: Request, res: Response) => {
    ...
    slotBooking = asyncHandler(async (req: Request, res: Response) => {
        const userId = req.userId;
        if (!userId) {
            res.status(STATUS_CODES.UNAUTHORIZED).json({ status: false, message: ERROR_MESSAGES.UNAUTHORIZED || "Unauthorized access", });
            return;
    ...
    getSessionsByUser = asyncHandler(async (req: Request, res: Response) => {
        const userId = req.userId;
        if (!userId) {
            res.status(STATUS_CODES.UNAUTHORIZED).json({ status: false, message: ERROR_MESSAGES.UNAUTHORIZED || 'Unauthorized access', });
            return
